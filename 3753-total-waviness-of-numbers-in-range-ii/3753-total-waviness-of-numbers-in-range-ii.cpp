...
public:
    ...
    pair<ll,ll> solve(int curr,int prevPrev,int prev,bool isLimitedwithActualNumber,bool isLeadingZero){
        ...
        for(int digit=0;digit<=limitDigit;digit++){
            ...
            auto [remainTotalNumbers,remainTotalWaveScore]=solve(curr+1,newPrevPrev,newPrev,
            isLimitedwithActualNumber && (digit==limitDigit),newIsLeadingZero);
    ...
    ll func(ll num){
        if(num<100)return 0;//atleast 3 digit to calculate waviness
